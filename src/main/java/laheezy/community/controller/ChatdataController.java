...
public class ChatdataController {
    ...
    public void enter(ChatDataRequestDto message, SimpMessageHeaderAccessor headerAccessor) {
        ...
        if (memberChatRoomService.checkMemberChatroom(member, room)) {
            ...
            List<Chatdata> allChatList = chatDataService.findAllChatByRoomIdAndDate(mcr.getChatroom(), mcr.getDate());
            if (allChatList != null) {
                for (Chatdata c : allChatList) {
                    chatList.add(convertToDto(c));
                }
            }
            // 해당 사용자에게만 메시지 전송
            String userId = headerAccessor.getUser().getName();
            sendingOperations.convertAndSendToUser(userId, "/chat/enter/" + message.getRoomId(), chatList);
        ...
        log.info(nowEnter.getMessage());

        //모든 사용자에게 전송
        sendingOperations.convertAndSend("/chat/enter/" + message.getRoomId(), save);
    }
