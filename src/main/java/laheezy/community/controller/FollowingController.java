...
import laheezy.community.controller.member.MemberController;
...
public class FollowingController {
    ...
    @Operation(summary = "memberId를 팔로우 한다.", tags = {"View"})
    public ResponseEntity<List<MemberResponseShortDto>> addFollwing(@PathVariable("memberId") Long memberId) {
        ...
        List<MemberResponseShortDto> shortDtos = follow.stream().map(o -> new MemberResponseShortDto(o.getMemberB().getNickname(), o.getMemberB().getId()))
    ...
    @Operation(summary = "나를 팔로잉 하는 사람 확인", tags = {"View"})
    public ResponseEntity<List<MemberResponseShortDto>>  getMyFollower() {
        ...
        List<MemberResponseShortDto> shortDtos = follower.stream().map(o -> new MemberResponseShortDto(o.getMemberB().getNickname(), o.getMemberB().getId()))
                .collect(Collectors.toList());
        return ResponseEntity.ok(shortDtos);
    }

    @GetMapping("/checkfollowing")
    @Operation(summary = "내가 팔로우 하는 사람 확인", tags = {"View"})
